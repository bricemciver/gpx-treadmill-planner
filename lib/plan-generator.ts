...
export function generatePlan(gpx: GPXData, treadmill: TreadmillParams, fitness: FitnessData): Plan {
  ...
  for (let i = 0; i < gpx.segments.length; i++) {
    ...
    const clampedIncline = clamp(rawIncline, treadmill.inclineMin_pct, treadmill.inclineMax_pct);
    const treadmillIncline = roundToStep(clampedIncline, treadmill.inclineStep_pct);
    const inclineCapped =
      rawIncline > treadmill.inclineMax_pct || rawIncline < treadmill.inclineMin_pct;
